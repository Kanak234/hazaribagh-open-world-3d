...
import { RealOsmLoader, ParsedOsmWorld } from '../gis/RealOsmLoader';
...
import { EngineTelemetry, HandlingConfig, PlayerInputState, Vector3D } from './Types';
...
export class EngineCoordinator {
  ...
  private loop: KalaChakraLoop;

  // Core Subsystems
  private octree: SpatialOctree;
  private physicsWorld: PhysicsWorld;
  private osmLoader: RealOsmLoader;
  private terrainGenerator: TerrainRoadGenerator;
  private roadGraph: RoadGraph;
  private pathfinder: PathfindingAStar;

  // Entities & Controllers
  private playerCharacter: CharacterController;
  private playerVehicle: RaycastVehicle;
  private playerVehicleEntity: VehicleEntity;
  private trafficDirector: TrafficDirector;
  private cameraController: ChaseCameraController;
  private hud: GameHUD;

  // Active World Data
  private worldData: ParsedOsmWorld | null = null;
  ...
  private readonly playerVehicleHandling: HandlingConfig = {
    id: 'hazaribagh_gt_v8',
  ...
  constructor() {
    ...
    const spawnPos: Vector3D = { x: 0, y: 1.5, z: 0 };
    ...
    const carSpawnPos: Vector3D = { x: 4.5, y: 1.5, z: 6.0 };
    ...
    this.loop = new KalaChakraLoop(
      (fixedDt) => this.physicsProcess(fixedDt),
      (delta, alpha) => this.renderProcess(delta, alpha)
    );
  ...
  private snapEntitiesToGround(): void {
   ...
   */
  private physicsProcess(fixedDt: number): void {
   ...
   */
  private renderProcess(delta: number, alpha: number): void {
    if (!this.playerCharacter || !this.playerVehicle) {
      return;
    }
    ...
    const fullTelemetry: EngineTelemetry = {
      fps: telemetry.fps,
  ...
  private findNearestLandmark(pos: THREE.Vector3): { name: string; distance: number } {
  ...
  private onKeyUp(e: KeyboardEvent): void {
    switch (e.code) {
      case 'KeyW':
      case 'ArrowUp':
        if (this.inputState.moveForward > 0) this.inputState.moveForward = 0;
        break;
      case 'KeyS':
      case 'ArrowDown':
        if (this.inputState.moveForward < 0) this.inputState.moveForward = 0;
      ...
      case 'KeyA':
      case 'ArrowLeft':
        if (this.inputState.moveRight < 0) this.inputState.moveRight = 0;
        break;
      case 'KeyD':
      case 'ArrowRight':
        if (this.inputState.moveRight > 0) this.inputState.moveRight = 0;
  ...
  public teleportToLandmark(nameQuery: string): void {
