...
import { AABB3D, RaycastHit, SweptHit, Vector3D } from '../core/Types';
...
export class PhysicsWorld {
  ...
  private colliders: Map<string, StaticCollider> = new Map();

  /**
   * Registers a static world collider (e.g., building, roadside structure).
   */
  public addCollider(collider: StaticCollider): void {
   ...
   */
  public sampleGround(x: number, z: number): { height: number; normal: Vector3D } {
   ...
   */
  public raycastGround(origin: Vector3D, maxDistance: number = 50): RaycastHit {
  ...
  public moveAndSlide(
  ...
  ): { finalDisplacement: Vector3D; hitCollider: boolean; groundContact: boolean } {
    let remaining = { ...displacement };
    let totalMoved: Vector3D = { x: 0, y: 0, z: 0 };
    let hitAny = false;
    let groundContact = false;

    let currentBox: AABB3D = {
      min: { ...box.min },
      max: { ...box.max },
    };
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      let closestHit: SweptHit | null = null;
