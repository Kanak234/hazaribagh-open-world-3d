...
export class RaycastVehicle {
    ...
    wheels = [];
    // Controls
    throttleInput = 0; // -1 (reverse) to 1 (accelerate)
    steeringInput = 0; // -1 (left) to 1 (right)
    handbrakeInput = false;
    currentSteerAngle = 0;
    constructor(config, physicsWorld, spawnPos) {
    ...
    physicsStep(fixedDt) {
        ...
        this.currentSteerAngle = THREE.MathUtils.damp(this.currentSteerAngle, targetSteer, this.config.steerSpeed, fixedDt);
        ...
        for (let i = 0; i < this.wheels.length; i++) {
            ...
            const groundHit = this.physicsWorld.raycastGround({ x: wheelMountWorld.x, y: wheelMountWorld.y, z: wheelMountWorld.z }, maxSuspensionDistance + this.config.wheelRadius);
            if (groundHit.hit) {
                ...
                let driveApplied = 0;
                const isDrivenWheel = this.config.driveType === 'AWD' ||
