...
interface CivilianCar {
  id: string;
  vehicle: RaycastVehicle;
  entity: VehicleEntity;
  targetNodeId: number | null;
  pathWaypoints: Vector3D[];
...
export class TrafficDirector {
  ...
  private civilians: CivilianCar[] = [];

  private readonly civilianHandling: HandlingConfig = {
    id: 'civilian_sedan',
   ...
   */
  public spawnCivilianFleet(count: number = 8): void {
   ...
   */
  public physicsStep(fixedDt: number, playerVehicle: RaycastVehicle | null, playerPos: THREE.Vector3): void {
   ...
   */
  public updateRender(alpha: number): void {
    for (const civ of this.civilians) {
      civ.entity.updateRender(alpha);
    }
  }

  private updateCivilianAI(civ: CivilianCar, playerVehicle: RaycastVehicle | null, playerPos: THREE.Vector3): void {
  ...
  private assignNewRandomDestination(civ: CivilianCar): void {
    const allNodes = this.roadGraph.getAllNodes();
    if (allNodes.length === 0) return;

    const startNode = this.roadGraph.findNearestNode(
      { x: civ.vehicle.position.x, y: civ.vehicle.position.y, z: civ.vehicle.position.z },
      200
    );
    if (!startNode) return;
  ...
  public getVehicles(): RaycastVehicle[] {
