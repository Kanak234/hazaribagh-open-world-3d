...
import { ParsedOsmWorld, ParsedRoadSegment, ParsedWaterBody } from './RealOsmLoader';
import { CoordinateProjection } from './CoordinateProjection';
import { SpatialOctree } from '../core/SpatialOctree';
import { AABB3D, LandmarkData } from '../core/Types';
...
export class TerrainRoadGenerator {
  ...
  public highwayMaterial: THREE.MeshStandardMaterial;
  private waterMaterial: THREE.MeshStandardMaterial;
  private terrainMaterial: THREE.MeshStandardMaterial;
  private hillMaterial: THREE.MeshStandardMaterial;
  private buildingMaterial: THREE.MeshStandardMaterial;
  private landmarkMaterial: THREE.MeshStandardMaterial;

  private roadMeshes: THREE.Mesh[] = [];
  private waterMeshes: THREE.Mesh[] = [];
  private terrainMeshes: THREE.Mesh[] = [];
  private buildingMeshes: THREE.InstancedMesh | null = null;
  private treeMeshes: THREE.InstancedMesh | null = null;
  private landmarkObjects: THREE.Group[] = [];

  constructor(scene: THREE.Scene, octree: SpatialOctree) {
   ...
   */
  public generateWorld(worldData: ParsedOsmWorld): void {
   ...
   */
  private generateTopographicTerrain(): void {
    const size = 6000;
    const segments = 120;
   ...
   */
  private generateRoadRibbons(roads: ParsedRoadSegment[]): void {
   ...
   */
  private generateWaterBodies(waterBodies: ParsedWaterBody[]): void {
   ...
   */
  private generateLandmarks(landmarks: LandmarkData[]): void {
   ...
   */
  private generateUrbanBuildingsAndVegetation(roads: ParsedRoadSegment[]): void {
    ...
    for (let r = 0; r < roads.length && bIndex < buildingCount; r++) {
      ...
      for (let p = 0; p < road.points.length && bIndex < buildingCount; p += 2) {
        ...
        if (Math.hypot(pt.x, pt.z) > 1800) continue;
    ...
    const canaryCenter = CoordinateProjection.gpsToWorld(
