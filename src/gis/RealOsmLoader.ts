...
import { OsmDataset, OsmWay, LandmarkData, Vector3D } from '../core/Types';
...
export interface ParsedOsmWorld {
  roads: ParsedRoadSegment[];
...
export class RealOsmLoader {
  private nodeMap: Map<number, { lat: number; lon: number; tags?: Record<string, string> }> = new Map();

  /**
   * Loads real OSM data from the cached static asset or queries the live Overpass API endpoint.
   */
  public async loadData(cacheUrl: string = '/data/hazaribagh_osm.json'): Promise<ParsedOsmWorld> {
    let dataset: OsmDataset | null = null;

    try {
      const response = await fetch(cacheUrl);
      if (response.ok) {
        dataset = (await response.json()) as OsmDataset;
      } else {
   ...
   */
  public async queryLiveOverpass(): Promise<OsmDataset> {
...
out skel qt;`;
    ...
    return (await response.json()) as OsmDataset;
  }

  /**
   * Parses raw OSM elements into structured 3D spatial representations.
   */
  public parseDataset(dataset: OsmDataset): ParsedOsmWorld {
    this.nodeMap.clear();
    ...
    console.log(
      `[RealOsmLoader] Parsed ${roads.length} road corridors, ${waterBodies.length} water bodies, and ${landmarks.length} cultural landmarks.`
    );
   ...
   */
  private ensureCoreLandmarks(landmarks: LandmarkData[]): void {
    const keyLandmarks = [
      {
