...
export class CoordinateProjection {
    ...
    static ORIGIN_LAT = 23.9924352;
    static ORIGIN_LON = 85.3616252;
    static ORIGIN_ELEVATION = 610.0; // Mean sea level (MSL) elevation in meters
    // Canary Hill real GPS coordinates (Observation Tower)
    static CANARY_HILL_LAT = 24.015021;
    static CANARY_HILL_LON = 85.385132;
    static CANARY_HILL_PEAK_ELEVATION = 675.0; // ~65m above urban plateau
    // Hazaribagh Jheel (Lake) Center GPS
    static LAKE_LAT = 23.996210;
    static LAKE_LON = 85.360140;
    // WGS-84 Equatorial Earth radius in meters
    static WGS84_A = 6378137.0;
    ...
    static METERS_PER_LAT = 110780.0; // meters per degree latitude at 24°N
    static METERS_PER_LON = CoordinateProjection.WGS84_A * Math.cos(CoordinateProjection.ORIGIN_LAT * CoordinateProjection.DEG_TO_RAD) * CoordinateProjection.DEG_TO_RAD;
    /**
     ...
     */
    static gpsToWorld(lat, lon, explicitElevation) {
     ...
     */
    static calculateTopographicElevation(worldX, worldZ) {
        // Canary Hill peak coordinates in local space
        const canaryWorld = CoordinateProjection.gpsToWorld(CoordinateProjection.CANARY_HILL_LAT, CoordinateProjection.CANARY_HILL_LON, CoordinateProjection.CANARY_HILL_PEAK_ELEVATION);
        ...
        const lakeWorld = CoordinateProjection.gpsToWorld(CoordinateProjection.LAKE_LAT, CoordinateProjection.LAKE_LON, CoordinateProjection.ORIGIN_ELEVATION);
